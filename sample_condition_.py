...
def main():
    ...
    for i, ref_img in enumerate(loader):
        ...
        def objective(trial):
            # 定义超参数搜索范围
            base_kl = trial.suggest_float('base_kl', 1e-5, 0.1)
            forget = trial.suggest_float('forget', 1e-5, 1)
            sigma = trial.suggest_float('sigma', 0.01, 0.1)
            base_lr = trial.suggest_float('base_lr', 0.01, 1)


            sample,accuracy = sample_fn(x_start=x_start, measurement=y_n, record=True, save_root=out_path,operator = operator, 
                                        base_kl=base_kl,forget=forget,sigma=sigma,base_lr=base_lr,label = ref_img*255)
            return accuracy
