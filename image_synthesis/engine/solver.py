...
class Solver(object):
        ...
        if self.debug == False: 
            for k, v in batch.items():
                if torch.is_tensor(v):
                    batch[k] = v.cuda() # v shape 4,3,256,256
        else:
            batch = batch[0].cuda()
        for op_sc_n, op_sc in self.optimizer_and_scheduler.items():
            if phase == 'train':
    ...
    def train_epoch(self):
        ...
        for itr, batch in enumerate(self.dataloader['train_loader']):
            ...
            self.last_iter += 1
            loss = self.step(batch, phase='train') # full train step is done here, optimizer, scheduler's step process. Return loss
            # logging info
